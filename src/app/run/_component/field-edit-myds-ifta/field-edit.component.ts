...
// Pre-processes the lookupList for ng-select to natively inject the `disabled` property ONLY if true
  ...
  onSurfaceClick(event: MouseEvent, f: any) {
    // 1. Data-driven Read-Only Guard
    ...
    switch (f.type) {
      case 'text':
        if (f.subType === 'richtext') {
          (container.querySelector('.angular-editor-textarea') as HTMLElement)?.focus();
        } else {
          (container.querySelector('input, textarea') as HTMLElement)?.focus();
        }
        break;

      case 'modelPicker':
      case 'select':
        container.querySelector('.ng-select-container')?.dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
        break;

      case 'date':
        if (f.subType === 'time') {
          (container.querySelector('ngb-timepicker input') as HTMLElement)?.focus();
        } else {
      ...
      case 'speech':
      case 'qr':
        // These all rely on a native or simulated click on their primary input
        (container.querySelector('input') as HTMLElement)?.click();
        break;

      case 'checkbox':
        // Specifically for the single checkbox toggle
        (container.querySelector('input[type="checkbox"]') as HTMLElement)?.click();
        break;
      ...
      // For static, simpleOption, checkboxOption, radio, btn, etc., 
      // we do nothing because they don't have a single central input to focus/click.
      default:
        break;
    }
  }
