...
// 1. Unified, Recursive Helper Method
  private decodeForm(form: any) {
    if (!form) return form;
    ...
    if (form.items) {
      Object.values(form.items).forEach((item: any) => {
        ...
        item.placeholder = item._placeholder; // Note: Kept original logic, no atobUTF here
        item.post = atobUTF(item._post, '@');
        item.pre = atobUTF(item._pre, '@');
      });
    }

    // Decode Sections & Tabs
    form.sections?.forEach((section: any) => section.pre = atobUTF(section._pre, '@'));
    form.tabs?.forEach((tab: any) => tab.pre = atobUTF(tab._pre, '@'));

    // Decode Tiers & Actions
    ...
    form.f = atobUTF(form._f, '@');
    form.onSave = atobUTF(form._onSave, '@');
    form.onSubmit = atobUTF(form._onSubmit, '@');
    form.onView = atobUTF(form._onView, '@');

    // Recursively decode previous form history
    if (form.prev) {
      this.decodeForm(form.prev);
    }

    return form;
  }

  // 2. Dramatically Simplified Main Methods
  getRunForm(id: number) {
    return this.http.get<any>(`${this.baseApi}/run/form/${id}`).pipe(
      map(res => this.decodeForm(res))
    );
  }
  ...
  getRunDataset(id: number) {
    return this.http.get<any>(`${this.baseApi}/run/dataset/${id}`).pipe(
      map(res => {
        // Re-use our unified form decoder
        if (res.form) {
          this.decodeForm(res.form);
        }

        // Decode Dataset-specific arrays
        res.items?.forEach((item: any) => {
          item.pre = atobUTF(item._pre, '@');
        });

        res.actions?.forEach((action: any) => {
          action.f = atobUTF(action._f, '@');
          action.pre = atobUTF(action._pre, '@');
        });
